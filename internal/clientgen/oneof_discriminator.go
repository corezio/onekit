...
func (g *Generator) generateFlattenedUnmarshal(
	...
	gf.P("if u, ok := any(variant).(interface{ UnmarshalJSONOnekit([]byte, protojson.UnmarshalOptions) error }); ok {")
	gf.P("if err := u.UnmarshalJSONOnekit(variantData, opts); err != nil {")
...
func (g *Generator) generateNestedUnmarshal(
	...
	gf.P("// Non-flattened unmarshal: forward opts to child's UnmarshalJSONOnekit if available")
	gf.P(`if variantRaw, exists := raw["`, fieldJSONName, `"]; exists {`)
	gf.P("variant := &", msgType, "{}")
	gf.P("if u, ok := any(variant).(interface{ UnmarshalJSONOnekit([]byte, protojson.UnmarshalOptions) error }); ok {")
	gf.P("if err := u.UnmarshalJSONOnekit(variantRaw, opts); err != nil {")
	gf.P(`return fmt.Errorf("failed to unmarshal variant %s: %%w", "`, fieldGoName, `", err)`)
	gf.P("}")
