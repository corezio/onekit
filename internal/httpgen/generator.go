...
func (g *Generator) generateService(gf *protogen.GeneratedFile, file *protogen.File, service *protogen.Service) error {
	...
	for i, method := range service.Methods {
		...
		gf.P()
		gf.P(handlerName, " = config.wrapHandler(", handlerName, ", RequestMetadata{")
		gf.P(`Service: "`, service.Desc.FullName(), `",`)
		gf.P(`Method: "`, method.Desc.Name(), `",`)
		gf.P(`Procedure: "`, method.Desc.FullName(), `",`)
		gf.P(`HTTPMethod: "`, httpMethod, `",`)
		gf.P(`PathPattern: "`, httpPath, `",`)
		gf.P("})")
...
func (g *Generator) generateServerOptionType(gf *protogen.GeneratedFile) {
	...
	gf.P("Service string")
	gf.P("Method string")
	gf.P("Procedure string")
	gf.P("HTTPMethod string")
	gf.P("PathPattern string")
	gf.P("}")
	gf.P()
	gf.P("// RequestResult describes the response produced by a generated handler.")
	gf.P("type RequestResult struct {")
	gf.P("StatusCode int")
	gf.P("BytesWritten int64")
	gf.P("Duration time.Duration")
	gf.P("}")
	gf.P()
	gf.P("// RequestObserver receives dependency-free request lifecycle callbacks.")
	gf.P("// RequestStarted may enrich the context passed to the handler and middleware.")
	gf.P("type RequestObserver interface {")
	gf.P("RequestStarted(context.Context, RequestMetadata) context.Context")
	gf.P("RequestFinished(context.Context, RequestMetadata, RequestResult)")
	gf.P("}")
	gf.P()
	gf.P("type requestMetadataContextKey struct{}")
	gf.P()
	gf.P("// RequestMetadataFromContext returns metadata for the matched generated route.")
	gf.P("func RequestMetadataFromContext(ctx context.Context) (RequestMetadata, bool) {")
	gf.P("metadata, ok := ctx.Value(requestMetadataContextKey{}).(RequestMetadata)")
	gf.P("return metadata, ok")
	gf.P("}")
	gf.P()
	gf.P("// Middleware wraps a generated HTTP handler. Middleware passed to WithMiddleware")
	gf.P("// is applied in declaration order, so the first middleware is the outermost.")
	gf.P("type Middleware func(http.Handler) http.Handler")
	gf.P()
	gf.P("// RequestIDGenerator creates a request ID when the configured request header is absent.")
	gf.P("type RequestIDGenerator func() string")
	gf.P()
	gf.P("type requestIDContextKey struct{}")
	gf.P()
	gf.P("// RequestIDFromContext returns the request ID installed by WithRequestID.")
	gf.P("func RequestIDFromContext(ctx context.Context) (string, bool) {")
	gf.P("requestID, ok := ctx.Value(requestIDContextKey{}).(string)")
	gf.P("return requestID, ok && requestID != \"\"")
	gf.P("}")
	gf.P()
	gf.P("func defaultRequestIDGenerator() string {")
	gf.P("var value [16]byte")
	gf.P("if _, err := rand.Read(value[:]); err != nil {")
	gf.P("return \"\"")
	gf.P("}")
	gf.P("return hex.EncodeToString(value[:])")
	gf.P("}")
	gf.P()
...
func (g *Generator) generateServerConfigurationStruct(gf *protogen.GeneratedFile) {
	...
	gf.P("middlewares []Middleware")
	gf.P("requestIDHeader string")
	gf.P("requestIDGenerator RequestIDGenerator")
	gf.P("observer RequestObserver")
	gf.P("}")
	gf.P()

	gf.P("func (c *serverConfiguration) wrapHandler(handler http.Handler, metadata RequestMetadata) http.Handler {")
	gf.P("for i := len(c.middlewares) - 1; i >= 0; i-- {")
	gf.P("if c.middlewares[i] != nil {")
	gf.P("handler = c.middlewares[i](handler)")
	gf.P("}")
	gf.P("}")
	gf.P("if c.observer != nil {")
	gf.P("handler = observationMiddleware(c.observer, metadata)(handler)")
	gf.P("}")
	gf.P("if c.requestIDHeader != \"\" {")
	gf.P("handler = requestIDMiddleware(c.requestIDHeader, c.requestIDGenerator)(handler)")
	gf.P("}")
	gf.P("handler = routeMetadataMiddleware(metadata)(handler)")
	gf.P("return handler")
	gf.P("}")
	gf.P()

	gf.P("func requestIDMiddleware(headerName string, generate RequestIDGenerator) Middleware {")
	gf.P("return func(next http.Handler) http.Handler {")
	gf.P("return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {")
	gf.P("requestID := r.Header.Get(headerName)")
	gf.P("if requestID == \"\" && generate != nil {")
	gf.P("requestID = generate()")
	gf.P("}")
	gf.P("if requestID != \"\" {")
	gf.P("w.Header().Set(headerName, requestID)")
	gf.P("r = r.WithContext(context.WithValue(r.Context(), requestIDContextKey{}, requestID))")
	gf.P("}")
	gf.P("next.ServeHTTP(w, r)")
	gf.P("})")
	gf.P("}")
	gf.P("}")
	gf.P()

	gf.P("func routeMetadataMiddleware(metadata RequestMetadata) Middleware {")
	gf.P("return func(next http.Handler) http.Handler {")
	gf.P("return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {")
	gf.P("ctx := context.WithValue(r.Context(), requestMetadataContextKey{}, metadata)")
	gf.P("next.ServeHTTP(w, r.WithContext(ctx))")
	gf.P("})")
	gf.P("}")
	gf.P("}")
	...
	gf.P("type observationResponseWriter struct {")
	gf.P("http.ResponseWriter")
	gf.P("statusCode int")
	gf.P("bytesWritten int64")
	gf.P("}")
	gf.P()
	gf.P("func (w *observationResponseWriter) WriteHeader(statusCode int) {")
	gf.P("if w.statusCode != 0 {")
	gf.P("return")
	gf.P("}")
	gf.P("w.statusCode = statusCode")
	gf.P("w.ResponseWriter.WriteHeader(statusCode)")
	gf.P("}")
	gf.P()
	gf.P("func (w *observationResponseWriter) Write(body []byte) (int, error) {")
	gf.P("if w.statusCode == 0 {")
	gf.P("w.statusCode = http.StatusOK")
	gf.P("}")
	gf.P("n, err := w.ResponseWriter.Write(body)")
	gf.P("w.bytesWritten += int64(n)")
	gf.P("return n, err")
	gf.P("}")
	gf.P()
	gf.P("func (w *observationResponseWriter) Flush() {")
	gf.P("if w.statusCode == 0 {")
	gf.P("w.WriteHeader(http.StatusOK)")
	gf.P("}")
	gf.P("if flusher, ok := w.ResponseWriter.(http.Flusher); ok {")
	gf.P("flusher.Flush()")
	gf.P("}")
	gf.P("}")
	gf.P()
	gf.P("func (w *observationResponseWriter) Unwrap() http.ResponseWriter {")
	gf.P("return w.ResponseWriter")
	gf.P("}")
	gf.P()

	gf.P("func observationMiddleware(observer RequestObserver, metadata RequestMetadata) Middleware {")
	gf.P("return func(next http.Handler) http.Handler {")
	gf.P("return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {")
	gf.P("ctx := observer.RequestStarted(r.Context(), metadata)")
	gf.P("if ctx == nil {")
	gf.P("ctx = r.Context()")
	gf.P("}")
	gf.P("started := time.Now()")
	gf.P("capture := &observationResponseWriter{ResponseWriter: w}")
	gf.P("defer func() {")
	gf.P("statusCode := capture.statusCode")
	gf.P("if statusCode == 0 {")
	gf.P("statusCode = http.StatusOK")
	gf.P("}")
	gf.P("observer.RequestFinished(ctx, metadata, RequestResult{")
	gf.P("StatusCode: statusCode,")
	gf.P("BytesWritten: capture.bytesWritten,")
	gf.P("Duration: time.Since(started),")
	gf.P("})")
	gf.P("}()")
	gf.P("next.ServeHTTP(capture, r.WithContext(ctx))")
	gf.P("})")
	gf.P("}")
	gf.P("}")
	gf.P()
...
func (g *Generator) generateServerOptions(gf *protogen.GeneratedFile) {
	...
	gf.P("// WithMiddleware applies middleware to every handler registered by the service.")
	gf.P("// The first middleware is the outermost. Nil middleware values are ignored.")
	gf.P("func WithMiddleware(middleware ...Middleware) ServerOption {")
	gf.P("return func(c *serverConfiguration) {")
	gf.P("c.middlewares = append(c.middlewares, middleware...)")
	gf.P("}")
	gf.P("}")
	gf.P()

	gf.P("// WithRequestObserver adds request lifecycle callbacks for tracing, logging, or metrics.")
	gf.P("func WithRequestObserver(observer RequestObserver) ServerOption {")
	gf.P("return func(c *serverConfiguration) {")
	gf.P("c.observer = observer")
	gf.P("}")
	gf.P("}")
	gf.P()

	gf.P("// WithRequestID enables request-ID propagation for every generated handler.")
	gf.P("// Existing inbound IDs are preserved; otherwise a random 128-bit ID is generated.")
	gf.P("// The ID is returned in the same response header and is available through RequestIDFromContext.")
	gf.P(`// An empty headerName uses "X-Request-ID".`)
	gf.P("func WithRequestID(headerName string) ServerOption {")
	gf.P("return WithRequestIDGenerator(headerName, defaultRequestIDGenerator)")
	gf.P("}")
	gf.P()

	gf.P("// WithRequestIDGenerator enables request IDs using a custom generator.")
	gf.P("// It is useful when IDs must come from an existing tracing or identity system.")
	gf.P("func WithRequestIDGenerator(headerName string, generate RequestIDGenerator) ServerOption {")
	gf.P("return func(c *serverConfiguration) {")
	gf.P("if headerName == \"\" {")
	gf.P(`headerName = "X-Request-ID"`)
	gf.P("}")
	gf.P("c.requestIDHeader = headerName")
	gf.P("c.requestIDGenerator = generate")
	gf.P("}")
	gf.P("}")
	gf.P()
}
...
func (g *Generator) generateDefaultErrorStatusCodeFunc(gf *protogen.GeneratedFile) {
	...
	gf.P("var handlerErr *onekithttp.Error")
	gf.P("if errors.As(err, &handlerErr) {")
	gf.P("return handlerErr.HTTPStatusCode()")
	gf.P("}")
